...
ttest b_h_edu if act_any==0, by(c_act)
...
di sqrt(0.3513641^2 + 0.6804326^2)
...
ttest b_h_edu if act_any==0, by(c_act) unequal
...
reg b_h_edu c_act if act_any==0
...
// answer:  it is the standard error from the ttest command that assumes equal variances across groups
...
reg b_h_edu c_act if act_any==0, robust
