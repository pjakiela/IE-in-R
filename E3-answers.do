...
ci means Rate1 if post==1
putexcel B4=`r(mean)', hcenter nformat(#.##)
local temp_se = string(r(se),"%03.2f")
putexcel B5="(`temp_se')", hcenter 

ci means Rate2 if post==0
putexcel C2=`r(mean)', hcenter nformat(#.##)
local temp_se = string(r(se),"%03.2f")
putexcel C3="(`temp_se')", hcenter 

ci means Rate2 if post==1
putexcel C4=`r(mean)', hcenter nformat(#.##)
local temp_se = string(r(se),"%03.2f")
putexcel C5="(`temp_se')", hcenter 
...
ttest Rate1, by(post)
return list
local temp_diff = r(mu_1) - r(mu_2)
putexcel B6=`temp_diff', hcenter nformat(#.##)
local temp_se = string(r(se),"%03.2f")
putexcel B7="(`temp_se')", hcenter 
...
ttest Rate2, by(post)
return list
local temp_diff = string(r(mu_1) - r(mu_2),"%03.2f")
putexcel C6="`temp_diff'", hcenter 
local temp_se = string(r(se),"%03.2f")
putexcel C7="(`temp_se')", hcenter 
...
ci means Rate1 if post==0
local mean_Div1 = r(mean)
local se_Div1 = r(se)

ci means Rate2 if post==0
local mean_Div2 = r(mean)
local se_Div2 = r(se)

local mean_diff = `mean_Div1' - `mean_Div2'
local se_diff = sqrt(`se_Div1'^2 + `se_Div2'^2)
di `mean_diff' 
di `se_diff'
...
local mean_diff = string(r(mu_1) - r(mu_2),"%03.2f")
putexcel D2="`mean_diff'", hcenter
local se_diff = string(r(se),"%03.2f")
putexcel D3="(`se_diff')", hcenter
...
ttest Rate1 = Rate2 if post==1, unpaired unequal 
local mean_diff = string(r(mu_1) - r(mu_2),"%03.2f")
putexcel D4="`mean_diff'", hcenter
local se_diff = string(r(se),"%03.2f")
putexcel D5="(`se_diff')", hcenter
...
ci mean Rate1 if post==0 
local txpre_mean = r(mean)
local txpre_se = r(se)

ci mean Rate1 if post==1 
local txpost_mean = r(mean)
local txpost_se = r(se)

ci mean Rate2 if post==0 
local cxpre_mean = r(mean)
local cxpre_se = r(se)

ci mean Rate2 if post==1 
local cxpost_mean = r(mean)
local cxpost_se = r(se)

local dd_est =  string(`txpost_mean' - `txpre_mean' - (`cxpost_mean' - `cxpre_mean'),"%03.2f")
putexcel D6="`dd_est'", hcenter
...
local dd_se = string(sqrt(`txpost_se'^2 + `txpre_se'^2 + `cxpost_se'^2 + `cxpre_se'^2),"%03.2f")
putexcel D7="(`dd_se')", hcenter
...
twoway (connected Rate1 Year, color(vermillion) msymbol(o) msize(small) lw(thin)) /// 
	(connected Rate2 Year, color(sea) msymbol(t) msize(small) lw(thin)), ///
	ylabel(0(5)20) ytitle("Maternal Mortality (Percent)" " ") ///
	xlabel(1840(5)1860) xtitle(" ") xline(1847) ///
	legend(label(1 "Doctors' Wing") label(2 "Midwives' Wing") col(1) ring(0) pos(2))
graph export vienna-by-wing-fig2.png, replace

** Export results using a matrix
...
svmat ddresults
putexcel F2=matrix(ddresults), hcenter nformat(#.##)
